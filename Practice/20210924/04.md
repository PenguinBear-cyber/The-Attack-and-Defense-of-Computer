# 4. Reversing using objdump

* objdump:在類Unix作業系統上顯示關於目的檔的各種資訊的命令列程式。例如，它可用作反組譯器來以組譯代碼形式檢視可執行檔。

以下指令為 objdump 反組譯程式碼並檢視可執行檔。

"-M intel":表示用 intel 語法展示。
```
...
同上，只是在指令後端加上"--no-show-raw-insn"。

"--no-show-raw-insn":表示反組譯時，不顯示彙編指令的機器碼。
```
